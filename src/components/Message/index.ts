...
class Message {
  ...
  constructor () {
    ...
    container.classList.add(baseClassName)
    this.ul.classList.add(wrapClassName)
    document.body.appendChild(container)
  }

  show (content: string, delay: number = 3000, type: string): void {
    const html = `
    <li class="${itemClassName} fade-in" data-id="${String(Message.id)}">
        <div>
             ${icon[type] as string}
            <span>${content}</span>
        </div>
    ...
    Message.id++
    const lis = document.querySelectorAll(`li.${itemClassName}`)
